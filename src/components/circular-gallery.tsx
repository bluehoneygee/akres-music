...
function createTextTexture(gl: unknown, text: string, font = "bold 30px monospace", color = "black") {
...
class Media {
  ...
  createMesh() {
    this.plane = new Mesh(this.gl as never, { geometry: this.geometry, program: this.program });
...
class App {
  ...
  constructor(
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend ?? 1, textColor, borderRadius, font);
    this.update();
  ...
  createMedias(items: GalleryItem[] | undefined, bend: number, textColor: string, borderRadius: number, font: string) {
    const defaultItems: GalleryItem[] = [{ image: "/akres-logo-full.png?v=6", text: "" }];
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.start = touchEvent.touches ? touchEvent.touches[0].clientX : (e as MouseEvent).clientX;
  }

  onTouchMove(e: MouseEvent | TouchEvent) {
    if (!this.isDown) return;
    const touchEvent = e as TouchEvent;
    const x = touchEvent.touches ? touchEvent.touches[0].clientX : (e as MouseEvent).clientX;
  ...
  onCheck() {
    if (!this.medias.length || !this.medias[0]) return;
